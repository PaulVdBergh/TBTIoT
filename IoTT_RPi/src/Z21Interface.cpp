...
*/
	...
	void Z21Interface::OnNewData(const mqttMessageQueueItem& item)
	{
		...
		{
			const char* szSubTopic = &(item.getTopic()[m_TopicBasePath.length()]);

			printf("Z21Interface::OnNewData : comparing ['Decoders/']\n\tto subTopic ['%s']\n", szSubTopic);

			if(0 == strncmp("Decoders/", szSubTopic, strlen("Decoders/")))
			{
				printf("Z21Interface::OnNewData : comparisons succeeded !!\n");

				const char* szAddress = &(szSubTopic[strlen("Decoders/")]);
	...
	void Z21Interface::threadFunc(void)
	{
		...
		int optVal = 1;
		setsockopt(m_fdsock_me, SOL_SOCKET, SO_REUSEADDR, &optVal, sizeof(optVal));
		setsockopt(m_fdsock_me, SOL_SOCKET, SO_REUSEPORT, &optVal, sizeof(optVal));
		...
		bind(m_fdsock_me, (const struct sockaddr*)&m_sockaddr_me, sizeof(m_sockaddr_me));
